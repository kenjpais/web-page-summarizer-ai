...
from filters.filter_required_fields import remove_irrelavant_fields_from_correlated
...
class TestCorrelateWithJiraIssueId(unittest.TestCase):
    def test_correlate_with_jira_issue_id(self):
        ...
        def normalize(d):
            d = dict(d)
            if "GITHUB" in d and isinstance(d["GITHUB"], list) and d["GITHUB"]:
                if "body" in d["GITHUB"][0]:
                    d["GITHUB"][0]["body"] = d["GITHUB"][0]["body"][:50]
            return d


        self.assertEqual(
            sorted(
                [normalize(item) for item in post_filter_data],
                key=lambda x: x["GITHUB"][0]["title"],
            ),
            sorted(
                [normalize(item) for item in relevant_correlated_data],
                key=lambda x: x["GITHUB"][0]["title"],
            ),
        )
